...
describe('inviteWhenLabel — 초대 시각 한 줄(앱 공통 날짜 문법)', () => {
  it('7월 9일 (목) 오후 2:00–3:00', () => {
    expect(inviteWhenLabel('2026-07-09', 840, 900)).toBe('7월 9일 (목) 오후 2:00–3:00');
  });
  it('오전/오후가 갈리면 끝 시각에도 접두어가 붙는다', () => {
    expect(inviteWhenLabel('2026-07-15', 690, 750)).toBe('7월 15일 (수) 오전 11:30–오후 12:30');
  });
});
...
describe('incomingInviteModel — 민수의 초대(INCOMING_INVITE) 그대로', () => {
  ...
  it('제목·시각·이유가 세계 데이터와 일치한다', () => {
    ...
    expect(model.whenLabel).toBe('7월 9일 (목) 오후 2:00–3:00 · 미팅룸 2');
...
describe('previewInviteModel — 데이터 소스 전환(binding 2)', () => {
  ...
  it('보낸 사람은 나(이찬), 수신자 관점 헤드라인·시각은 조정 반영 시간', () => {
    ...
    expect(preview.model.whenLabel).toBe('7월 15일 (수) 오전 10:00–11:00');
  });
