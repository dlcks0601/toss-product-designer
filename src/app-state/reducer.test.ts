...
describe('ADD_MY_EVENT — 혼자 경로의 개인 일정 저장', () => {
  ...
  it('meeting kind는 focus로 강등된다 — 회의는 시간 찾기 경로에서만 태어난다', () => {
    const next = reducer(initialState(), {
      type: 'ADD_MY_EVENT',
      event: { ...event, kind: 'meeting' },
    });
    expect(next.myEvents[0].kind).toBe('focus');
  });

  it('폼이 고른 종류(집중·외근·점심·휴가)는 존중된다', () => {
    for (const kind of ['focus', 'offsite', 'lunch', 'personal'] as const) {
...
describe('RESPOND_INVITE — 여정 B 응답(수락→캘린더 반영 / 거절→플래그만)', () => {
  it('참석할게요 — 반복 초대 시리즈 전체(매주 목)가 내 캘린더에 meeting으로 자리 잡는다', () => {
    const next = reducer(initialState(), { type: 'RESPOND_INVITE', response: 'accepted' });
    expect(next.inviteResponded).toBe('accepted');
    expect(next.myEvents).toHaveLength(INCOMING_INVITE.days.length);
    for (const [i, day] of INCOMING_INVITE.days.entries()) {
      expect(next.myEvents[i]).toMatchObject({
        day,
  ...
  it('응답은 1회 — 이미 응답했다면 no-op(중복 myEvents 추가 방지)', () => {
    ...
    expect(again.myEvents).toHaveLength(INCOMING_INVITE.days.length);
  ...
  it('수락 회의는 기존 myEvents(확정 회의·개인 일정) 뒤에 쌓인다', () => {
    ...
    expect(next.myEvents.map((e) => e.id)).toEqual([
      ...
      ...INCOMING_INVITE.days.map((day) => `invite-${day}T${INCOMING_INVITE.start}`),
    ]);
  });
