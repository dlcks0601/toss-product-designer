...
/** 이벤트 팩토리 — 결정적 id. */
function ev(id: string, day: string, start: number, end: number, title: string, kind: EventKind): CalendarEvent {
  return { id, day, start, end, title, kind };
}

// ── 핵심 6인 ───────────────────────────────────────────────

/**
 * 이찬(나) — PM·주최자. lunchRhythm 12:30~13:10(매일 점심). 보통의 미팅 부하.
...
const ichan: Person = {
  ...
  events: [
    ...
    ev('ichan-w0-1', W0_MON, 690, 780, '주간 스프린트 계획', 'meeting'),
    ev('ichan-w0-2', W0_TUE, 660, 780, '제품 리뷰', 'meeting'),
    ...
    ev('ichan-w1-l1', W1_MON, 750, 790, '점심', 'lunch'),
    ev('ichan-w1-1', W1_MON, 810, 930, '팀 미팅', 'meeting'),
    ev('ichan-w1-l2', W1_TUE, 750, 790, '점심', 'lunch'),
    ev('ichan-w1-2', W1_TUE, 600, 720, '제품 리뷰', 'meeting'),
    ev('ichan-w1-3', W1_TUE, 960, 1080, '스폰서 리뷰', 'meeting'),
    ev('ichan-w1-l3', W1_WED, 750, 790, '점심', 'lunch'),
    ev('ichan-w1-3b', W1_WED, 900, 1080, '분기 리뷰', 'meeting'),
    ev('ichan-w1-l4', W1_THU, 750, 790, '점심', 'lunch'),
    ev('ichan-w1-4', W1_THU, 900, 1020, '채용 인터뷰', 'meeting'),
    ev('ichan-w1-l5', W1_FRI, 750, 790, '점심', 'lunch'),
    ev('ichan-w1-5', W1_FRI, 810, 930, '위클리 랩업', 'meeting'),

    // ── flexible ──
    ev('ichan-w2-l1', W2_MON, 750, 790, '점심', 'lunch'),
    ev('ichan-w2-1', W2_MON, 690, 780, '주간 스프린트 계획', 'meeting'),
    ev('ichan-w2-l2', W2_WED, 750, 790, '점심', 'lunch'),
...
const junho: Person = {
  ...
  events: [
    ...
    ev('junho-w0-l0', W0_MON, 720, 780, '점심', 'lunch'),
    ev('junho-w0-2', W0_TUE, 600, 720, '프론트 팀 싱크', 'meeting'),
    ev('junho-w0-l1', W0_TUE, 720, 780, '점심', 'lunch'),
 ...
 * → 목 11:00 슬롯이 lunch-squeeze. S3 소재.
 */
const seoyeon: Person = {
  ...
  events: [
    ...
    ev('seoyeon-w1-l5', W1_FRI, 720, 780, '점심', 'lunch'),
    ev('seoyeon-w1-5', W1_FRI, 690, 810, '리서치 싱크', 'meeting'),

    // ── flexible ──
    ev('seoyeon-w2-l1', W2_TUE, 720, 780, '점심', 'lunch'),
...
const minsu: Person = {
  ...
  events: [
    ...
    // ── 다음 주(next-week) ──
    ev('minsu-w1-l1', W1_MON, 720, 780, '점심', 'lunch'),
    ev('minsu-w1-1b', W1_MON, 810, 1080, '포트폴리오 리뷰', 'meeting'),
    ev('minsu-w1-l2', W1_TUE, 720, 780, '점심', 'lunch'),
    ev('minsu-w1-2', W1_TUE, 600, 840, '디자인 워크숍', 'meeting'),
    ev('minsu-w1-l3', W1_WED, 720, 780, '점심', 'lunch'),
    ...
    ev('minsu-w1-l4', W1_THU, 720, 780, '점심', 'lunch'),
    ev('minsu-w1-4b', W1_THU, 780, 960, '디자인 시스템 정리', 'meeting'),
    ev('minsu-w1-l5', W1_FRI, 720, 780, '점심', 'lunch'),
    ev('minsu-w1-5', W1_FRI, 780, 960, '브랜드 리뷰', 'meeting'),

    // ── flexible ──
    ev('minsu-w2-l1', W2_WED, 720, 780, '점심', 'lunch'),
...
const haneul: Person = {
  ...
  events: [
    ...
    ev('haneul-w1-l4', W1_THU, 780, 820, '점심', 'lunch'),
    // Fri : 오전을 채워 불참
    ev('haneul-w1-5', W1_FRI, 600, 780, '제휴 미팅', 'meeting'),
    ev('haneul-w1-l5', W1_FRI, 780, 820, '점심', 'lunch'),
...
/**
 * 오세훈 — BE(선택). lunchRhythm 13:00~13:40(매일 점심) + 촘촘한 일정 → 화 14:00 시작 = after-lunch 발동.
  ...
  for (const wd of weekdays) {
    ...
    for (const [wi, day] of [d0, d1, d2].entries()) {
      ...
      for (const [mwd, ms, me, title] of s.meetings) {
        if (mwd === wd) events.push(ev(`${s.id}-m-${mwd}-${ms}-${wi}`, day, ms, me, title, 'meeting'));
      }
      // 외근
...
const EXTRA_SPECS: ExtraSpec[] = [
  // 2 offsite-요일 people (코어와 다른 요일: 수·금)
  {
    ...
    workHours: { start: 540, end: 1080 }, lunch: [720, 780],
  ...
  },
  {
    ...
    workHours: { start: 540, end: 1080 }, lunch: [780, 840],
  ...
  },
  {
    ...
    workHours: { start: 540, end: 1080 }, lunch: [750, 810],
  ...
  // 3 lunch-rhythm people (11:30 / 12:00 / 13:30)
  {
    id: 'saebom', name: '윤새봄', role: 'UX Writer', faceId: 'saebom',
    workHours: { start: 540, end: 1080 }, lunch: [690, 750],
  ...
  // 2 early-birds (workHours 480~1020)
  {
    ...
    workHours: { start: 480, end: 1020 }, lunch: [690, 750],
  ...
  // 2 flexible (workHours 600~1140)
  {
    ...
    workHours: { start: 600, end: 1140 }, lunch: [780, 840],
